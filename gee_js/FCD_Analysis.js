// ============================================================
// Forest Canopy Density (FCD) in Pekalongan Regency
// ============================================================
...
// 2. LANDSAT 9 → CLOUD MASKING & SCALING
function maskL9sr(image) {
  var qa = image.select('QA_PIXEL');
  var cloud = qa.bitwiseAnd(1 << 2).neq(0) // cirrus
  ...
  // Scaling optical bands (Collection 2 L2 scaling)
...
var l9_fix = l9.median().clip(roi);

// Visualisasi RGB
Map.addLayer(l9_fix, {bands:['SR_B4','SR_B3','SR_B2'], min:0, max:0.3}, 'RGB Composite');

// 3. SPECTRAL INDICES
var ndvi = l9_fix.normalizedDifference(['SR_B5','SR_B4']).rename('NDVI');

var L = 0.5;
var savi = l9_fix.expression(
  '((NIR - RED)/(NIR + RED + L))*(1+L)',
  {'NIR': l9_fix.select('SR_B5'), 'RED': l9_fix.select('SR_B4'), 'L': L}
).rename('SAVI');

var bi = l9_fix.expression(
  '((SWIR + RED) - (NIR + BLUE))/((SWIR + RED) + (NIR + BLUE))',
  {'SWIR': l9_fix.select('SR_B6'),'RED': l9_fix.select('SR_B4'),'NIR': l9_fix.select('SR_B5'),'BLUE': l9_fix.select('SR_B2')}
).rename('BI');

var ndmi = l9_fix.normalizedDifference(['SR_B5','SR_B6']).rename('NDMI');
...
print('NDVI percentiles raw (3 & 98):', ndviVals);

// Ambil nilai percentile (dengan fallback sederhana jika null)
var p3 = ee.Number(ndviVals.get('NDVI_p3'));
var p98 = ee.Number(ndviVals.get('NDVI_p98'));

// Jika salah satu null, berikan fallback aman
p3 = ee.Algorithms.If(p3, p3, -0.1); p3 = ee.Number(p3);
p98 = ee.Algorithms.If(p98, p98, 0.9); p98 = ee.Number(p98);
...
// Konversi ke persen & klasifikasi (4 kelas sesuai permintaan)
var fcd_percent = fcd.multiply(100).rename('FCD_Percent');

var fcd_class_raw = fcd_percent.expression(
  "b('FCD_Percent') < 20 ? 1 : (b('FCD_Percent') < 40 ? 2 : (b('FCD_Percent') < 70 ? 3 : 4))",
  {'FCD_Percent': fcd_percent}
).rename('FCD_Class');

// Pastikan hanya piksel yang punya nilai FCD yang tampil (masking) dan clip ke ROI
var fcd_class = fcd_class_raw
                  .updateMask(fcd_percent.mask())   // hanya tampilkan piksel yg memiliki mask pada fcd_percent
                  .clip(roi)                        // opsional: pastikan terbatas pada ROI
                  .rename('FCD_Class');

Map.addLayer(fcd_class, {min:1, max:4, palette:['ffffcc','c7e9b4','a1dab4','238443']}, 'FCD Class');
...
// Fungsi bantu ekspor (Drive)
function exportImage(image, name) {
  Export.image.toDrive({
    image: image.toFloat(),
    description: name + '_Export',
    folder: 'GEE_exports',
    fileNamePrefix: name + '_2024',
    region: roi.geometry(),
    scale: 30,
    maxPixels: 1e13
  });
...
// Ekspor hasil
exportImage(ndvi, 'NDVI');
exportImage(savi, 'SAVI');
exportImage(bi, 'BI');
exportImage(ndmi, 'NDMI');
exportImage(vd_smoothed, 'VD_Smoothed');
exportImage(s_index, 'Shadow_Index');
exportImage(vd_fused, 'VD_Fused');
exportImage(fcd, 'FCD');
exportImage(fcd_percent, 'FCD_Percent');
